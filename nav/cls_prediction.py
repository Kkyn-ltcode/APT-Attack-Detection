    ...
    if state.trained_model is not None:
        ...
        if add_selectbox == "Online":
            with st.spinner("Predicting ..."):
                input_df = retrieve_train_element(X_before_preprocess, target_name, ignore_columns,"Classification")
                output = ""
                if st.button("Predict"):
                    output = online_predict(trained_model, input_df,"Classification")
                    output = str(output)
