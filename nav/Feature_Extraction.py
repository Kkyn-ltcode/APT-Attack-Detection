...
# Function to generate features from a DataFrame of events using word embeddings
def generate_event(df):
    WORD_VECTORS_PATH = os.path.join('Model/Event/Word Vector', 'event.model')
    if os.path.isfile(WORD_VECTORS_PATH):
        word_vectors = load_word_vectors(WORD_VECTORS_PATH)

    # Data preprocessing for event information
    df['Info'] = df['Info'].apply(lambda x: re.sub(r'\[(?:[^\]|]*\|)?([^\]|]*)\]', '', x))
    df['Info'] = df['Info'].apply(lambda x: re.sub(r'--', ' ', x))
    df['Info'] = df['Info'].apply(lambda x: re.sub(r'-', ' ', x))
    df['Info'] = df['Info'].apply(lambda x: re.sub(r': ', ' ', x))
    df['Info'] = df['Info'].apply(lambda x: re.sub(r';', ' ', x))
    df['Info'] = df['Info'].apply(lambda x: ' '.join(x.split()))
    df['Info'] = df['Info'].apply(lambda x: ' '.join(x.split('\\')))
    df.drop_duplicates(ignore_index=True, inplace=True)

    # Word embedding features for event information
